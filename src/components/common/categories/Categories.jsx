      ...
      <header className="fixed top-0 w-full h-16 flex items-center justify-center bg-white z-10 mb-4">
        {selectedGroupId ? (
          <>
           ...
           <Button onClick={()=> setSelectedGroupId(null)} className="flex items-center justify-end  px-2 gap-1 flex-row-reverse">
      ...
      <main className="grid grid-cols-2 gap-2 px-4 py-20">
        ...
        {groups.mGroup.map((category) => (
          <CategoriesCard
            ...
            onClick={() => handleGroupClick(category.groupId)} // Attach click handler
          />
        ))}
      </main>
      {selectedGroupId && (
        <div className="fixed top-20 left-[50%] -translate-x-[50%] w-full bg-white grid grid-cols-2 gap-2 px-2">
          ...
          {filteredSGroups.map((sGroup) => (
            <CategoriesCard
              ...
              onClick={() => handleGroupClick(sGroup.groupId)} // Attach click handler
            />
          ))}
        </div>
      )}
